...
<style>
    ...
    .stSlider > div > div > div > div {
        background: linear-gradient(90deg, #ef4444 0%, #ef4444 53%, #f59e0b 53%, #f59e0b 67%, #10b981 67%, #10b981 77%, #3b82f6 77%);
    }
...
def calcular_sueldo_actual(sueldo_fijo, comisiones, bono_productividad, bono_regular, efectividad):
    """
    Esquema Actual: Sueldo Fijo + (Comisiones + Bono Productividad) × % Efectividad + Bono Regular
    """
    variable_ajustado = (comisiones + bono_productividad) * (efectividad / 100)
    return sueldo_fijo + variable_ajustado + bono_regular

# Función para calcular sueldo esquema nuevo
def calcular_sueldo_nuevo(sueldo_fijo, comisiones, bono_productividad, bono_regular, efectividad):
    """
    Esquema Nuevo: Sueldo Fijo + (Comisiones + Bono Productividad + Bono Regular) × % Pago Tabla
    """
    porcentaje_pago = obtener_porcentaje_pago(efectividad)
    sueldo_variable = comisiones + bono_productividad + bono_regular
...
with st.sidebar:
    ...
    comisiones = st.number_input(
        ...
        value=1500, 
        step=100,
        help="Comisiones al 100% de cumplimiento"
    )
    
    bono_productividad = st.number_input(
        "🎯 Bono Productividad (S/.)", 
        min_value=0, 
        value=1000, 
        step=100,
        help="Bono por productividad al 100%"
    )
    
    bono_regular = st.number_input(
        "🏆 Bono Regular (S/.)", 
        min_value=0, 
        value=500, 
        step=100,
        help="Bono fijo regular"
    )
    
    st.markdown("---")
    
    total_al_100 = sueldo_fijo + comisiones + bono_productividad + bono_regular
    sueldo_variable_total = comisiones + bono_productividad + bono_regular
    
    st.markdown(f"""
    <div class="info-box">
        <strong>📋 Resumen de Compensación</strong><br>
        <small>
        • Sueldo Variable Total: <b>S/. {sueldo_variable_total:,.0f}</b><br>
        • Total al 100%: <b>S/. {total_al_100:,.0f}</b>
        </small>
    </div>
    """, unsafe_allow_html=True)
...
# Cálculos
sueldo_actual = calcular_sueldo_actual(sueldo_fijo, comisiones, bono_productividad, bono_regular, efectividad)
sueldo_nuevo = calcular_sueldo_nuevo(sueldo_fijo, comisiones, bono_productividad, bono_regular, efectividad)
...
sueldos_actuales = [calcular_sueldo_actual(sueldo_fijo, comisiones, bono_productividad, bono_regular, e) for e in efectividades]
sueldos_nuevos = [calcular_sueldo_nuevo(sueldo_fijo, comisiones, bono_productividad, bono_regular, e) for e in efectividades]
...
# Puntos Clave del Nuevo Esquema - Justo después del gráfico
...
with col_k2:
    ...
    <div style="background: #fef3c7; padding: 16px; border-radius: 12px; text-align: center;">
        <div style="font-weight: 700; color: #f59e0b; font-size: 1.1rem;">80-99%</div>
...
with col_k3:
    ...
    <div style="background: #d1fae5; padding: 16px; border-radius: 12px; text-align: center;">
        <div style="font-weight: 700; color: #10b981; font-size: 1.1rem;">100-114%</div>
...
st.markdown("### 📊 Desglose de tu Compensación Actual")

col1, col2 = st.columns(2)

with col1:
    # Esquema Actual - Desglose
    componentes_actual = {
        "Sueldo Fijo": sueldo_fijo,
        "Comisiones": comisiones * (efectividad/100),
        "Bono Productividad": bono_productividad * (efectividad/100),
        "Bono Regular": bono_regular
    }
    ...
    fig_actual = go.Figure(go.Waterfall(
        ...
        x=["Sueldo Fijo", "Comisiones", "Bono Prod.", "Bono Regular", "TOTAL"],
        y=[sueldo_fijo, comisiones * (efectividad/100), bono_productividad * (efectividad/100), bono_regular, 0],
        text=[f"S/. {v:,.0f}" for v in [sueldo_fijo, comisiones * (efectividad/100), bono_productividad * (efectividad/100), bono_regular, sueldo_actual]],
    ...
    fig_actual.update_layout(
        title=dict(text="<b>Esquema Actual</b>", font=dict(size=14)),
        showlegend=False,
        height=350,
...
with col2:
    ...
    if efectividad >= 80:
        ...
        fig_nuevo = go.Figure(go.Waterfall(
            ...
            measure=["relative", "relative", "relative", "relative", "total"],
            x=["Sueldo Fijo", "Comisiones", "Bono Prod.", "Bono Regular", "TOTAL"],
            y=[sueldo_fijo, comisiones * porcentaje, bono_productividad * porcentaje, bono_regular * porcentaje, 0],
            text=[f"S/. {v:,.0f}" for v in [sueldo_fijo, comisiones * porcentaje, bono_productividad * porcentaje, bono_regular * porcentaje, sueldo_nuevo]],
            textposition="outside",
...
for e in escenarios:
    s_actual = calcular_sueldo_actual(sueldo_fijo, comisiones, bono_productividad, bono_regular, e)
    s_nuevo = calcular_sueldo_nuevo(sueldo_fijo, comisiones, bono_productividad, bono_regular, e)
...
for e in range(0, 151):
    s_a = calcular_sueldo_actual(sueldo_fijo, comisiones, bono_productividad, bono_regular, e)
    s_n = calcular_sueldo_nuevo(sueldo_fijo, comisiones, bono_productividad, bono_regular, e)
    if s_n >= s_a and e >= 80:
        punto_equilibrio = e
        break
